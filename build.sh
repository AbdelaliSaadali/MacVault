...
# both Apple Silicon and Intel Macs. VORSSAINT_ARCH can be set explicitly when
# cross-compiling with a macOS SDK (accepted values: arm64, x86_64).
TARGET_ARCH="${VORSSAINT_ARCH:-$(uname -m)}"
case "$TARGET_ARCH" in
    arm64|x86_64) ;;
    *)
        echo "✗ Unsupported Mac architecture: $TARGET_ARCH" >&2
        echo "  Set VORSSAINT_ARCH to arm64 or x86_64." >&2
        exit 1
        ;;
esac
