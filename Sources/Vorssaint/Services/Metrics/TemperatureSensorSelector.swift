...
enum TemperatureSensorSelector {
    ...
    static func platform(brandString: String?) -> CPUTemperaturePlatform {
        ...
        default:
            if brand.hasPrefix("Apple ") { return .unmappedAppleSilicon }
            if brand.range(of: "Intel", options: .caseInsensitive) != nil { return .intel }
            return .generic
        }
    }
    ...
    static func isCPUCoreKey(_ key: String, platform: CPUTemperaturePlatform) -> Bool {
        ...
        case .unmappedAppleSilicon, .intel, .generic:
            return false
        }
    }
    ...
    static func isCPUTemperatureKey(_ key: String,
        ...
        case .unmappedAppleSilicon:
            return false
        case .intel:
            // Intel Macs conventionally expose CPU package/core/diode/proximity
            // temperatures as TCxx keys (for example TC0P, TC0D, TC0H, TC1C).
            return key.hasPrefix("TC")
        case .appleM3Family:
            return key.hasPrefix("Tp") || key.hasPrefix("Te") || key.hasPrefix("Tf")
        case .appleM1Family, .appleM2Family, .appleM4Family, .appleM5Family:
            return key.hasPrefix("Tp") || key.hasPrefix("Te")
        case .generic:
            return key.hasPrefix("Tp") || key.hasPrefix("Te") || key.hasPrefix("TC")
        }
    }
    ...
    static func isGPUTemperatureKey(_ key: String,
        ...
        switch platform {
        case .intel:
            // Intel-era SMCs use uppercase TGxx keys, while Apple Silicon uses Tgxx.
            return key.hasPrefix("TG")
        case .appleM1Family, .appleM2Family, .appleM3Family, .appleM4Family,
             .appleM5Family, .unmappedAppleSilicon:
            return key.hasPrefix("Tg")
        case .generic:
            return key.hasPrefix("Tg") || key.hasPrefix("TG")
        }
    }
