...
final class FanControlHardware {
    ...
    private enum FanModeControl {
        /// Apple Silicon and some newer Intel Macs expose one byte per fan.
        case perFan(SMCClient.Key)
        /// Classic Intel SMCs expose a shared `FS! ` bitmask; bit N forces fan N.
        case forceMask(SMCClient.Key)
    }
    ...
    private var cachedForceMaskKey: SMCClient.Key?
    private var didDiscoverForceMaskKey = false
    ...
    func startCooling(level: Int) throws -> [FanControlFanReading] {
        let fans = try discoverControlledFans()
        guard try fans.allSatisfy({ try !isManuallyControlled($0) }) else {
            throw FanControlHardwareError.alreadyControlled
        }
        if let forceTest = forceTestKey(), try byteValue(forceTest) != 0 {
            throw FanControlHardwareError.alreadyControlled
        ...
        if directSucceeded {
            ...
            directSucceeded = fans.allSatisfy { (try? isManuallyControlled($0)) == true }
        }
        ...
        if !directSucceeded {
            ...
            for fan in fans {
                ...
                guard setManualMode(true, for: fan, untilUptime: deadline) else {
                    throw FanControlHardwareError.operationFailed
                }
            }
        }
    ...
    func restoreAutomatic() -> Bool {
        ...
        for fan in fans {
            _ = setManualMode(false, for: fan, attempts: 20)
        ...
        let automatic = fans.allSatisfy { fan in
            (try? isManuallyControlled(fan)) == false
        }
    ...
    private func discoverControlledFans() throws -> [Fan] {
        ...
        for index in 0..<count {
            guard let actual = client.key(named: "F\(index)Ac"),
                  ...
                  let mode = modeControl(for: index),
    ...
    private func modeControl(for index: Int) -> FanModeControl? {
        // Classic Intel Macs normally use the shared FS! bitmask. Prefer it on
        // Intel even when a per-fan mode key happens to exist, because FS! is
        // the control path used by the Intel SMC firmware.
        if temperaturePlatform == .intel, let key = forceMaskKey() {
            return .forceMask(key)
        }
        for name in ["F\(index)md", "F\(index)Md"] {
            if let key = client.key(named: name), key.dataSize == 1, client.readBytes(key) != nil {
                return .perFan(key)
            }
        }
        if let key = forceMaskKey() { return .forceMask(key) }
        return nil
    }

    private func forceMaskKey() -> SMCClient.Key? {
        if didDiscoverForceMaskKey { return cachedForceMaskKey }
        didDiscoverForceMaskKey = true
        guard let key = client.key(named: "FS! "),
              let value = client.readValue(key),
              value.isFinite, value >= 0, value.rounded() == value,
              SMCValueCodec.encode(value, type: key.dataType, size: Int(key.dataSize)) != nil else {
            return nil
        }
        cachedForceMaskKey = key
        return key
    }
    ...
    private func readings(for fans: [Fan]) throws -> [FanControlFanReading] {
        try fans.map { fan in
            ...
            return FanControlFanReading(index: fan.index,
                                        ...
                                        isManuallyControlled: try isManuallyControlled(fan))
        }
    }
    ...
    private func verifyCooling(_ fans: [Fan], targets: [Double], attempts: Int = 1) -> Bool {
        ...
        for attempt in 0..<attempts {
            let matches = zip(fans, targets).allSatisfy { fan, expected in
                guard (try? isManuallyControlled(fan)) == true,
    ...
    private func isManuallyControlled(_ fan: Fan) throws -> Bool {
        switch fan.mode {
        case .perFan(let key):
            return !FanControlPolicy.isAutomaticMode(try modeValue(key))
        case .forceMask(let key):
            let bit = UInt32(1) << UInt32(fan.index)
            return (try forceMaskValue(key) & bit) != 0
        }
    }

    private func forceMaskValue(_ key: SMCClient.Key) throws -> UInt32 {
        guard let value = client.readValue(key), value.isFinite, value >= 0,
              value.rounded() == value, value <= Double(UInt32.max) else {
            throw FanControlHardwareError.operationFailed
        }
        return UInt32(value)
    }
    ...
    private func setManualMode(_ manual: Bool, for fan: Fan, attempts: Int) -> Bool {
        switch fan.mode {
        case .perFan(let key):
            let value: UInt8 = manual ? 1 : 0
            guard setByte(value, for: key, attempts: attempts) else { return false }
            return (try? modeValue(key)) == value
        case .forceMask(let key):
            let bit = UInt32(1) << UInt32(fan.index)
            for attempt in 0..<attempts {
                do {
                    let current = try forceMaskValue(key)
                    let updated = manual ? (current | bit) : (current & ~bit)
                    try client.writeValue(Double(updated), to: key)
                    if let forced = try? forceMaskValue(key), ((forced & bit) != 0) == manual {
                        return true
                    }
                } catch {}
                if attempt + 1 < attempts { Thread.sleep(forTimeInterval: 0.05) }
    ...
    private func setTargetRPM(_ target: Double, for fan: Fan, attempts: Int) -> Bool {
        for attempt in 0..<attempts {
            ...
            if let currentTarget = client.readValue(fan.target),
               ...
               (try? isManuallyControlled(fan)) == true {
                return true
            }
            if attempt + 1 < attempts { Thread.sleep(forTimeInterval: 0.05) }
        }
    ...
    private func writeManualPair(for fan: Fan, target: Double) -> Bool {
        guard setManualMode(true, for: fan, attempts: 1) else { return false }
        do {
    ...
    private func setManualMode(_ manual: Bool, for fan: Fan, untilUptime deadline: TimeInterval) -> Bool {
        repeat {
            if setManualMode(manual, for: fan, attempts: 1) { return true }
