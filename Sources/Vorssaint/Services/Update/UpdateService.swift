...
final class UpdateService: ObservableObject {
    ...
    /// Upstream release bundles are currently built arm64-only. An Intel port
    /// must not replace itself with one of those bundles. Keep the updater
    /// active on the architecture the upstream release pipeline actually ships;
    /// Intel builds stay on their locally built binary until an Intel/universal
    /// release source is configured.
    private static var officialSelfUpdateSupported: Bool {
#if arch(arm64)
        true
#else
        false
    ...
    func startAutomaticChecks() {
        ...
        guard Self.officialSelfUpdateSupported else {
            state = .upToDate
            availableNotes = nil
            return
        }
    ...
    private func configureAutomaticChecks() {
        ...
        refreshTimer = nil
        guard Self.officialSelfUpdateSupported, autoCheckEnabled else { return }
    ...
    func checkIfStale(maxAge: TimeInterval = 15 * 60) {
        if !Self.officialSelfUpdateSupported || AppInfo.isDeveloperBuild { return }
    ...
    func downloadAndInstall() {
        if !Self.officialSelfUpdateSupported || AppInfo.isDeveloperBuild { return }
        // Never replace an Intel port with the upstream arm64-only release.
